...
export const loginUser = async (email, password) => {
  try {
    ...
    const errorMessage =
      errorData.mensagem || errorData.message || "Erro ao conectar ao servidor";
...
export const updateCategory = async (id_category, categoryData) => {
  try {
    const response = await api.put(
      `/categoria/atualizar/${id_category}`,
      categoryData,
    );
    return response.data;
  } catch (error) {
...
export const createTypeOfPayment = async (paymentData) => {
  try {
    ...
    throw (
      error.response?.data || { mensagem: "Erro ao criar tipo de pagamento" }
    );
  }
};
...
export const updateTypeOfPayment = async (id_type_of_payment, paymentData) => {
  try {
    const response = await api.put(
      `/pagamento/atualizar/${id_type_of_payment}`,
      paymentData,
    );
    return response.data;
  } catch (error) {
    ...
    throw (
      error.response?.data || {
        mensagem: "Erro ao atualizar tipo de pagamento",
      }
    );
  }
};
...
export const deleteTypeOfPayment = async (id_type_of_payment) => {
  try {
    ...
    throw (
      error.response?.data || { mensagem: "Erro ao excluir tipo de pagamento" }
    );
  }
};
